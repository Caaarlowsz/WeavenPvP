...
public class WarpHandler
{
    ...
    public static void defaultItens(final Player player, final boolean compass) {
        ...
        final ItemStack compassItemStack = new ItemBuilder().name("§e§lBussola").type(Material.COMPASS).amount(1).build();
        for (int i = 1; i < 36; ++i) {
    ...
    public static class Spawn extends ListenerHandler
    {
        ...
        public void onDamageCamper(final EntityDamageByEntityEvent event) {
            if (event.getEntity() instanceof Player && event.getDamager() instanceof Player) {
                ...
                if (Handler.getManager().getProtectionManager().isProtected(damager) && !Handler.getManager().getProtectionManager().isProtected(entity) && !gamer.getWarp().equalsIgnoreCase("1v1")) {
                    ...
                    if (localClickedItem.getType() == Material.CHEST) {
                        ...
                        for (final Kit localKit : Handler.getManager().getKitManager().getAllKits()) {
                            if (localClickedItem.getType() == localKit.getMaterial()) {
                                ...
                                localPlayer.sendMessage("§2Voc\u00ea selecionou o kit §f" + localKit.getName().toUpperCase());
                                final TitleAPI title = new TitleAPI("§e§lKIT " + localKit.getName().toUpperCase(), "§eSelecionado!");
        ...
        public static void setItensToPlayer(final Player playerToSet) {
            ...
            final ItemStack chestItemStack = new ItemBuilder().name("§eSeletor de Kits §7(Clique para utilizar)").type(Material.CHEST).build();
            final ItemStack compassItemStack = new ItemBuilder().name("§eSeletor de Warps §7(Clique para utilizar)").type(Material.PAPER).build();
            final ItemStack diamondItemStack = new ItemBuilder().name("§eStatus §7(Clique para utilizar)").type(Material.GOLD_INGOT).build();
        ...
        public static void inventoryOfWarps(final Player playerToOpen) {
            final Inventory localInventory = Bukkit.createInventory((InventoryHolder)playerToOpen, 9, "§7Menu de Warps");
            final ItemStack glassItemStack = new ItemBuilder().name("§e§lFPS").lore("", "§f" + getInWarp("Fps") + "§2 jogando agora", "").type(Material.GLASS).build();
            final ItemStack blazeItemStack = new ItemBuilder().name("§e§l1v1").lore("", "§f" + getInWarp("1v1") + "§2 jogando agora", "").type(Material.BLAZE_ROD).build();
            final ItemStack lavaItemStack = new ItemBuilder().name("§e§lLava Challenge").lore("", "§f" + getInWarp("Challenge") + "§2 jogando agora", "").type(Material.LAVA_BUCKET).build();
            final ItemStack cookietemStack = new ItemBuilder().name("§e§lRei da Mesa").lore("", "§f" + getInWarp("Rdm") + "§2 jogando agora", "").type(Material.COOKIE).build();
        ...
        public static void firstInventoryOfKits(final Player playerToOpen) {
            final Inventory localInventory = Bukkit.createInventory((InventoryHolder)playerToOpen, 54, "§7Selecione seu kit (1)");
            ...
            for (final Kit kitOfSession : linkedListOfKit) {
                ...
                if (kitOfSession.needPermission() && playerToOpen.hasPermission("pvpkit." + kitOfSession.getName().toLowerCase()) && integerOfMaxSize < linkedListOfInteger.size()) {
                    localInventory.setItem((int)linkedListOfInteger.get(integerOfMaxSize), kitOfSession.getItemStackMenu());
                    ++integerOfMaxSize;
                }
            ...
            final ItemStack chestItemStack = new ItemBuilder().name("§7Seus kits (in\u00edcio)").type(Material.CHEST).build();
            final ItemStack greenSackItemStack = new ItemBuilder().name(nextPage ? "§cDescer" : "§aDescer").type(Material.INK_SACK).data(nextPage ? 1 : 10).build();
            final ItemStack redSackItemStack = new ItemBuilder().name("§cSubir").type(Material.INK_SACK).data(1).build();
        ...
        public static void secondInventoryOfKits(final Player playerToOpen) {
            final Inventory localInventory = Bukkit.createInventory((InventoryHolder)playerToOpen, 54, "§7Selecione seu kit (2)");
            final LinkedList<Integer> linkedListOfInteger = new LinkedList<Integer>();
            for (int i = 11; i < 17; ++i) {
                linkedListOfInteger.add(i);
            }
            ...
            Spawn.mapOfLinkedList.put(playerToOpen.getUniqueId(), linkedLitOfKitToSave);
            final ItemStack chestItemStack = new ItemBuilder().name("§7Seus kits (in\u00edcio)").type(Material.CHEST).build();
            final ItemStack redSackGrayItemStack = new ItemBuilder().name("§cDescer").type(Material.INK_SACK).data(1).build();
            final ItemStack greenSackGrayItemStack = new ItemBuilder().name("§aSubir").type(Material.INK_SACK).data(10).build();
            localInventory.setItem(18, chestItemStack);
