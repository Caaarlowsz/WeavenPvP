...
public final class X1WarpListener implements Listener {
	...
	public void onInteract(final PlayerInteractEvent e) {
		...
		if (i.getType() == Material.getMaterial(351)) {
			if (i.getItemMeta().getDisplayName().equals("�e�l1v1 R\u00e1pido")) {
				...
				e.getPlayer().updateInventory();
				e.getPlayer()
						.sendMessage("�eO 1v1 R\u00e1pido est\u00e1 procurando algu\u00e9m para voc\u00ea batalhar!");
	...
	public void onChallengeInteract(final PlayerInteractEntityEvent e) {
		...
		final Gamer g1 = WeavenPvP.getManager().getGamerManager().getGamer(e.getPlayer().getUniqueId());
		final Gamer g2 = WeavenPvP.getManager().getGamerManager().getGamer(challenged.getUniqueId());
		final ItemStack i = e.getPlayer().getItemInHand();
		if (g1.getWarp().equalsIgnoreCase("1v1") && g2.getWarp().equalsIgnoreCase("1v1")) {
			if (e.getPlayer().getItemInHand().getType() == Material.BLAZE_ROD
					&& i.getItemMeta().getDisplayName().equals("�6�l1v1 Normal")) {
				...
				if (X1WarpListener.challenge.containsKey(challenged)
						&& X1WarpListener.challenge.get(challenged) == e.getPlayer()) {
			...
			if (e.getPlayer().getItemInHand().getType() == Material.IRON_FENCE
					&& i.getItemMeta().getDisplayName().equals("�b�l1v1 Customizado")) {
				if (X1WarpListener.cooldown.contains(e.getPlayer())) {
					e.getPlayer().sendMessage("�eAguarde para desafiar novamente");
					return;
				}
				if (X1WarpListener.challengec.containsKey(challenged)
						&& X1WarpListener.challengec.get(challenged) == e.getPlayer()) {
					if (X1WarpListener.firstMatch == e.getPlayer().getUniqueId()) {
						X1WarpListener.firstMatch = null;
					}
					if (X1WarpListener.firstMatch == challenged.getUniqueId()) {
	...
	public static String cora(final Player p) {
		...
		return NumberFormat.getCurrencyInstance().format(vida.getHealth() / 2.0).replace("$", "").replace("R", "")
				.replace(",", ".");
	}
	...
	public void onQuit(final PlayerQuitEvent e) {
		...
		if (X1WarpListener.playerfigh.containsKey(e.getPlayer())) {
			...
			final Gamer g1 = WeavenPvP.getManager().getGamerManager().getGamer(w.getUniqueId());
			final Gamer g2 = WeavenPvP.getManager().getGamerManager().getGamer(e.getPlayer().getUniqueId());
			...
			defaultItens(w);
			w.teleport(WeavenPvP.getManager().getWarps().getLocation("1v1"));
	...
	public static ItemStack newItemEnchant(final Material material, final Enchantment ench, final int qnt,
			final byte color) {
		final ItemStack i = new ItemStack(material, qnt, (short) color);
		final ItemMeta ik = i.getItemMeta();
	...
	public static final void loadWarp1v1Methods(final Player bp) {
		WeavenPvP.getManager().getProtectionManager().setProtected(bp, true);
		final Gamer gamer = WeavenPvP.getManager().getGamerManager().getGamer(bp.getUniqueId());
		defaultItens(bp);
		gamer.setWarp("1v1");
	}
	...
	public static void defaultItens(final Player p) {
		...
		gamer.setWarp("1v1");
		WeavenPvP.getManager().getProtectionManager().setProtected(p, true);
		p.getInventory().clear();
		p.getInventory().setArmorContents((ItemStack[]) null);
	...
	public static void teleportToFight(final Player p1, final Player p2) {
		WeavenPvP.getManager().getProtectionManager().setProtected(p1, false);
		WeavenPvP.getManager().getProtectionManager().setProtected(p2, false);
		p1.teleport(WeavenPvP.getManager().getWarps().getLocation("1v1loc1"));
		p2.teleport(WeavenPvP.getManager().getWarps().getLocation("1v1loc2"));
		prepareInventory(p1);
		prepareInventory(p2);
		update1v1Vanish(p1);
		update1v1Vanish(p2);
