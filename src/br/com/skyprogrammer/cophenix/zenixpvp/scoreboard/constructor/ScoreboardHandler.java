...
public class ScoreboardHandler {
	...
	public ScoreboardHandler() {
		...
		this.objectiveOfScoreboard = ScoreboardAPI.registerNewObjectiveToScoreboard(this.scoreboardToCreate, "sidebar",
				"dummy", DisplaySlot.SIDEBAR, "");
	...
	public void updateLine(final int index, final String text) {
		...
		final Team teamFromScoreboard = ScoreboardAPI.getTeamFromScoreboard(this.scoreboardToCreate,
				"handlerTeam-" + index);
		if (text.length() < 16) {
			...
			if (text.length() > 16 && text.length() <= 32) {
				teamFromScoreboard
						.setSuffix(String.valueOf(ChatColor.getLastColors(text.substring(0, 16))) + text.substring(16));
			} else if (text.length() > 16 && text.length() > 32) {
				teamFromScoreboard.setSuffix(String.valueOf(ChatColor.getLastColors(text.substring(0, 16)))
						+ text.substring(16).substring(0, 16));
			}
		}
	}
}
