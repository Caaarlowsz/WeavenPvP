	...
	public boolean execute(final CommandSender commandSender, final String commandLabel, final String[] commandArgs) {
		...
		if (commandArgs.length == 0) {
			localPlayer.sendMessage(
					"�e�lFEAST�f Utilize: /feastcontrol <stop>, <addchest>, <removechest>, <start> ou <settime> + <time>");
			return true;
		}
		if (commandArgs.length == 1) {
			if (commandArgs[0].equalsIgnoreCase("stop")) {
				WeavenPvP.getManager().getFeastManager().stop();
				localPlayer.sendMessage("�2Voc\u00ea �fCANCELOU�2 o feast caso estivesse em progresso.");
				return true;
			}
			if (commandArgs[0].equalsIgnoreCase("start")) {
				WeavenPvP.getManager().getFeastManager().stop();
				WeavenPvP.getManager().getFeastManager().start();
			...
			if (commandArgs[0].equalsIgnoreCase("addchest")) {
				final Block targetBlock = localPlayer.getTargetBlock(null, 200);
				if (targetBlock.getType() != Material.CHEST) {
					localPlayer.sendMessage("�cVoc\u00ea n\u00e3o est\u00e1 �fOLHANDO�c para um �fBAU�c!");
					return true;
				}
				WeavenPvP.getManager().getFeast().addChest(targetBlock.getLocation());
				...
				WeavenPvP.getManager().getFeast().removeChest(targetBlock.getLocation());
				...
				if (commandArgs[0].equalsIgnoreCase("settime")) {
					...
					if (!isSecond && !isMinute && !isHour) {
						try {
							...
							WeavenPvP.getManager().getFeastManager().setTimer(integerOfTime);
							commandSender.sendMessage("�2Voc\u00ea �fALTEROU�2 o tempo para �f" + commandArgs[1]);
							return true;
						} catch (NumberFormatException localException) {
							commandSender.sendMessage(
									"�e�lTEMPO�f Para definir o tempo sem utilizar 's' ou 'm' voc\u00ea deve usar apenas numeros.");
							return true;
						}
					}
					if (isSecond) {
						try {
							final int integerOfTime = Integer.valueOf(commandArgs[1].replace("s", "").replace("S", ""))
									* 1;
							WeavenPvP.getManager().getFeastManager().setTimer(integerOfTime);
							commandSender.sendMessage("�2Voc\u00ea �fALTEROU�2 o tempo para �f" + commandArgs[1]);
							return true;
							...
							commandSender.sendMessage(
									"�e�lTEMPO�f Para definir segundos use apenas 1 's' como string; e coloque 's' no final do valor tempo escolhido. Ex: 10s");
							return true;
						}
					}
					if (isMinute) {
						try {
							final int integerOfTime = 60
									* Integer.valueOf(commandArgs[1].replace("m", "").replace("M", ""));
							WeavenPvP.getManager().getFeastManager().setTimer(integerOfTime + 1);
							commandSender.sendMessage("�2Voc\u00ea �fALTEROU�2 o tempo para �f" + commandArgs[1]);
							return true;
						} catch (NumberFormatException localException) {
							commandSender.sendMessage(
									"�e�lTEMPO�f Para definir minutos use apenas 1 'm' como string; e coloque 'm' no final do valor tempo escolhido. Ex: 10m");
							return true;
						}
					}
					if (!isHour) {
					...
					try {
						final int integerOfTime = 3600
								* Integer.valueOf(commandArgs[1].replace("h", "").replace("H", ""));
						WeavenPvP.getManager().getFeastManager().setTimer(integerOfTime);
						commandSender.sendMessage("�2Voc\u00ea �fALTEROU�2 o tempo para �f" + commandArgs[1]);
						return true;
					} catch (NumberFormatException localException) {
						commandSender.sendMessage(
								"�e�lTEMPO�f Para definir horas use apenas 1 'h' como string; e coloque 'h' no final do valor tempo escolhido. Ex: 10h");
						return true;
					}
				}
				localPlayer.sendMessage(
