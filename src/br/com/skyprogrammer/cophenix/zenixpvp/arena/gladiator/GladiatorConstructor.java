...
public class GladiatorConstructor
{
    ...
    public GladiatorConstructor(final int arenaSize, final Location arenaLocation, final Material arenaMaterial, final int tentatives) {
    ...
    public GladiatorConstructor generate() {
        for (int blockX = -this.arenaSize; blockX <= this.arenaSize; ++blockX) {
            for (int blockZ = -this.arenaSize; blockZ <= this.arenaSize; ++blockZ) {
                for (int blockY = -1; blockY <= this.arenaSize; ++blockY) {
                    this.blocks.add(this.arenaLocation.clone().add((double)blockX, (double)blockY, (double)blockZ).getBlock());
                    final Block blocksLocation = this.arenaLocation.clone().add((double)blockX, (double)blockY, (double)blockZ).getBlock();
                    if (!blocksLocation.isEmpty()) {
                        final Random random = new Random();
                        final int x = random.nextInt(this.arenaLocation.getBlockX() + this.arenaSize + this.tentatives * 5);
                        final int z = random.nextInt(this.arenaLocation.getBlockZ() + this.arenaSize + this.tentatives * 5);
                        final Location otherLocation = new Location(this.arenaLocation.getWorld(), (double)x, (double)(this.arenaLocation.getBlockY() + -this.tentatives), (double)z);
                        final GladiatorConstructor gladiator = new GladiatorConstructor(this.arenaSize, otherLocation, this.arenaMaterial, this.tentatives + 1);
                        return gladiator;
                    }
                    if (blockY == this.arenaSize) {
                        this.arenaBlocks.add(this.arenaLocation.clone().add((double)blockX, (double)blockY, (double)blockZ));
                    }
                    else if (blockY == -1) {
                        this.arenaBlocks.add(this.arenaLocation.clone().add((double)blockX, (double)blockY, (double)blockZ));
                    }
                    else if (blockX == -this.arenaSize || blockZ == -this.arenaSize || blockX == this.arenaSize || blockZ == this.arenaSize) {
                        this.arenaBlocks.add(this.arenaLocation.clone().add((double)blockX, (double)blockY, (double)blockZ));
                    }
                }
            }
        ...
        (this.side1 = new Location(this.arenaLocation.getWorld(), this.arenaLocation.getX() + 8.0, this.arenaLocation.getY() + 1.0, this.arenaLocation.getZ() + 8.0)).setYaw(135.0f);
        (this.side2 = new Location(this.arenaLocation.getWorld(), this.arenaLocation.getX() - 8.0, this.arenaLocation.getY() + 1.0, this.arenaLocation.getZ() - 8.0)).setYaw(-45.0f);
        return this;
    }
        ...
        this.whiterTask = new BukkitRunnable() {
            public void run() {
                if (GladiatorConstructor.this.player1 != null) {
                    GladiatorConstructor.this.player1.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, 10000, 2));
                }
                if (GladiatorConstructor.this.player2 != null) {
                    GladiatorConstructor.this.player2.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, 10000, 2));
                }
            }
