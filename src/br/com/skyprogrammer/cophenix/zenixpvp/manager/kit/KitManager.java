...
public class KitManager
{
    ...
    public KitManager loadKits(final String stringOfThePackageNameToLoad) {
        ...
        int error = 0;
        try {
            for (final Class<?> classConstructor : ClassGetter.getClassesForPackage(this.instanceOfHandler, stringOfThePackageNameToLoad)) {
                if (Kit.class.isAssignableFrom(classConstructor)) {
                    try {
                        Kit instanceOfKit = null;
                        try {
                            final Constructor<?> instanceOfClassConstructor = classConstructor.getConstructor(this.instanceOfHandler.getClass());
                        ...
                        this.instanceOfHandler.getServer().getPluginManager().registerEvents((Listener)instanceOfKit, (Plugin)this.instanceOfHandler);
                        ++sucess;
                    }
