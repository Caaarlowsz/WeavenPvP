...
public abstract class Kit implements Listener
{
    ...
    public Kit(final String kitName, final Material kitMaterial, final int dataByte, final int kitPrice, final int kitCooldown, final boolean kitNeedPermission, final String... kitDescription) {
    ...
    public ItemStack getItemStackMenu() {
        ...
        if (this.kitName == "Bow") {
            localKitItemStack = new ItemBuilder().name("§a" + this.kitName).type(this.kitMaterial).data(this.dataByte).lore((String[])listOfKitDescription.toArray(new String[0])).enchant(Enchantment.ARROW_DAMAGE, 5).enchant(Enchantment.ARROW_INFINITE, 5).enchant(Enchantment.ARROW_FIRE, 5).build();
        }
        else if (this.kitName == "PvP") {
            localKitItemStack = new ItemBuilder().name("§a" + this.kitName).type(this.kitMaterial).data(this.dataByte).lore((String[])listOfKitDescription.toArray(new String[0])).enchant(Enchantment.DAMAGE_ALL, 1).build();
        }
        else if (this.kitName == "Grandpa") {
            localKitItemStack = new ItemBuilder().name("§a" + this.kitName).type(this.kitMaterial).data(this.dataByte).lore((String[])listOfKitDescription.toArray(new String[0])).enchant(Enchantment.KNOCKBACK, 2).build();
        }
        else {
            localKitItemStack = new ItemBuilder().name("§a" + this.kitName).type(this.kitMaterial).data(this.dataByte).lore((String[])listOfKitDescription.toArray(new String[0])).build();
        }
