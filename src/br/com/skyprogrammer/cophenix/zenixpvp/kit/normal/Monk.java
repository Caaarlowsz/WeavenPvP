...
{
    public Monk() {
        super("Monk", Material.BLAZE_POWDER, 0, 3000, 12, true, new String[] { "", "Ao usar seu item bagun\u00e7e o inventario", "de seus oponentes os embolando", "e tendo a chance para mat\u00e1-los." });
    }
    ...
    public void onPlayerInteractMonk(final PlayerInteractEntityEvent localPlayerInteractEntityEvent) {
        ...
        if (localPlayerInteractEntityEvent.getRightClicked() instanceof Player) {
            ...
            if (localPlayer.getItemInHand().getType() == Material.BLAZE_ROD && localGamer.getKit() == this) {
                ...
                final int integerWhitRandomValue = new Random().nextInt(localRightClickedPlayer.getInventory().getSize() - 10 + 1 + 10);
                final ItemStack randomItemStackOfInventory = localRightClickedPlayer.getInventory().getItem(integerWhitRandomValue);
                ...
                localPlayer.sendMessage("§2§l" + this.getName() + "§2 Voc\u00ea §fMONKOU§2 o player §f" + localRightClickedPlayer.getName());
                localRightClickedPlayer.sendMessage("§c§l" + this.getName() + "§c O player §f" + localPlayer.getName() + "§c acabou de §fBAGUN\u00c7AR§c seu §fINVENTARIO!");
            }
        }
    }
}
