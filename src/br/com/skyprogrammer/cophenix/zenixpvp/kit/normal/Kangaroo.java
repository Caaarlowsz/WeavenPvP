...
public class Kangaroo extends Kit
{
    ...
    public Kangaroo() {
        super("Kangaroo", Material.FIREWORK, 0, 7500, 7, true, new String[] { "", "Com seu foguete se locomova mais", "r\u00e1pido pelo mapa e impe\u00e7a que", "seus inimigos fujam de voc\u00ea." });
    }
    ...
    public void onInteract(final PlayerInteractEvent localPlayerInteractEvent) {
        ...
        if ((localGamer.getKit() == this & localPlayerInteractEvent.getAction() != Action.PHYSICAL) && localPlayer.getItemInHand().getType() == this.getMaterial()) {
    ...
    public void onMove(final PlayerMoveEvent localPlayerMoveEvent) {
        if (Kangaroo.listOfKangaroo.contains(localPlayerMoveEvent.getPlayer().getUniqueId()) && (localPlayerMoveEvent.getPlayer().getLocation().getBlock().getRelative(BlockFace.DOWN).getType() != Material.AIR || localPlayerMoveEvent.getPlayer().isOnGround())) {
