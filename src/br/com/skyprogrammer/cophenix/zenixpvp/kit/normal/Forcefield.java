...
public class Forcefield extends Kit
{
    ...
    public Forcefield() {
        super("Forcefield", Material.NETHER_FENCE, 0, 14000, 50, true, new String[] { "", "Ao usar seu item ganhe as", "'habilidades' do hack forcefield e tenha", "um range de 6 blocos." });
    ...
    public void onInteractForcefield(final PlayerInteractEvent localPlayerInteractEvent) {
        ...
        if (localGamer.getKit() == this) {
            if (localPlayer.getItemInHand().getType() != this.getMaterial()) {
                if (this.arrayListOfForcefield.contains(localPlayer)) {
                    for (final Entity entities : localPlayer.getNearbyEntities(6.0, 6.0, 6.0)) {
                        ...
                        if (entities instanceof Damageable) {
                            ...
                            ((Player)entities).sendMessage("§c§l" + this.getName() + "§c Voc\u00ea est\u00e1 no §fFORCEFIELD§c do player §f" + localPlayer.getName());
                        }
                    }
                }
            }
                ...
                localPlayer.sendMessage("§2§l" + this.getName() + "§2 Voc\u00ea §fATIVOU§2 seu §fFORCEFIELD!§2 Bata com §fESPADA§2 para dar mais §fDANO!");
                Handler.getInstance().getServer().getScheduler().scheduleSyncDelayedTask((Plugin)Handler.getInstance(), () -> this.removeAbilityIfHas(localPlayer), 300L);
            }
        }
    }
}
