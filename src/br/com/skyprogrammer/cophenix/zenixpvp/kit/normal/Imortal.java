...
public class Imortal extends Kit
{
    ...
    public Imortal() {
        super("Imortal", Material.BEDROCK, 0, 9500, 44, true, new String[] { "", "Use seu item e fique completamente", "invuner\u00e1vel \u00e1 danos por 10 segundos." });
    ...
    public void onInteractImortal(final PlayerInteractEvent localPlayerInteractEvent) {
        ...
        if (localGamer.getKit() == this && localPlayer.getItemInHand().getType() == this.getMaterial()) {
            ...
            Handler.getInstance().getServer().getScheduler().scheduleSyncDelayedTask((Plugin)Handler.getInstance(), () -> this.removeAbilityIfHas(localPlayer), 200L);
        }
    }
