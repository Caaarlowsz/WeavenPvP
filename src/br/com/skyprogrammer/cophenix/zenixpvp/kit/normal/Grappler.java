...
public class Grappler extends Kit
{
    ...
    public Grappler() {
        super("Grappler", Material.LEASH, 0, 10000, 7, true, new String[] { "", "Tenha um la\u00e7o com uma corda que", "agarra em qualquer coisa podendo", "ir ou escalar em qualquer lugar." });
    ...
    private void onInteractGrappler(final PlayerInteractEvent localPlayerInteractEvent) {
        ...
        if (player.getItemInHand().getType() == this.getMaterial() && localGamer.getKit() == this) {
            ...
            if (localPlayerInteractEvent.getAction() == Action.LEFT_CLICK_AIR || localPlayerInteractEvent.getAction() == Action.LEFT_CLICK_BLOCK) {
                ...
                final GrapplerHook localGrapplerHook = new GrapplerHook(player.getWorld(), (EntityHuman)((CraftPlayer)player).getHandle());
                localGrapplerHook.spawn(player.getEyeLocation().add(((Vector)localObject).getX(), ((Vector)localObject).getY(), ((Vector)localObject).getZ()));
                localGrapplerHook.move(5.0 * ((Vector)localObject).getX(), 5.0 * ((Vector)localObject).getY(), 5.0 * ((Vector)localObject).getZ());
                this.hashMapOfGrappler.put(player.getUniqueId(), localGrapplerHook);
            }
            else if (this.hashMapOfGrappler.containsKey(player.getUniqueId()) && this.hashMapOfGrappler.get(player.getUniqueId()).isHooked()) {
                final Object localObject = this.hashMapOfGrappler.get(player.getUniqueId()).getBukkitEntity().getLocation();
    ...
    public void onGrapplerFallDamage(final EntityDamageEvent localEntityDamageEvent) {
        if (localEntityDamageEvent.getEntity() instanceof Player) {
            ...
            final Gamer localGamer = Handler.getManager().getGamerManager().getGamer(localPlayer.getUniqueId());
            if (localGamer.getKit() == this && localEntityDamageEvent.getCause() == EntityDamageEvent.DamageCause.FALL && localEntityDamageEvent.getDamage() > 12.0) {
