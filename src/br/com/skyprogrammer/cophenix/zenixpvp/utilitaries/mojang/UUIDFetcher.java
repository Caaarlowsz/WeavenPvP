...
public class UUIDFetcher {
	...
	static {
		UUIDFetcher.gson = new GsonBuilder().registerTypeAdapter((Type) UUID.class, (Object) new UUIDTypeAdapter())
				.create();
	...
	public static UUID getUUIDAt(String name, final long timestamp) {
		...
		try {
			final HttpURLConnection connection = (HttpURLConnection) new URL(
					String.format("https://api.mojang.com/users/profiles/minecraft/%s?at=%d", name, timestamp / 1000L))
							.openConnection();
			connection.setReadTimeout(6000);
			final UUIDFetcher data = (UUIDFetcher) UUIDFetcher.gson.fromJson(
	...
	public static String getName(final UUID uuid) {
		...
		try {
			...
			final UUIDFetcher[] nameHistory = (UUIDFetcher[]) UUIDFetcher.gson.fromJson(
					new BufferedReader(new InputStreamReader(connection.getInputStream())), UUIDFetcher[].class);
