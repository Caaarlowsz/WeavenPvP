...
public class GrapplerHook extends EntityFishingHook
{
    ...
    public void h() {
        ...
        for (final Entity entity : this.instanceofEntitySnowBall.world.getWorld().getEntities()) {
            if (!(entity instanceof Firework) && entity.getEntityId() != this.getBukkitEntity().getEntityId() && entity.getEntityId() != this.instanceOfEntityHuman.getBukkitEntity().getEntityId() && entity.getEntityId() != this.instanceofEntitySnowBall.getBukkitEntity().getEntityId()) {
                if (entity.getLocation().distance(this.instanceofEntitySnowBall.getBukkitEntity().getLocation()) >= 2.0) {
                    if (!(entity instanceof Player)) {
                        continue;
                    }
                    ((Player)entity).getEyeLocation().distance(this.instanceofEntitySnowBall.getBukkitEntity().getLocation());
                }
                else {
    ...
    public void spawn(final Location location) {
        this.instanceOfSnowBall = (Snowball)this.instanceOfEntityHuman.getBukkitEntity().launchProjectile((Class)Snowball.class);
        this.instanceofEntitySnowBall = ((CraftSnowball)this.instanceOfSnowBall).getHandle();
        final PacketPlayOutEntityDestroy packet = new PacketPlayOutEntityDestroy(new int[] { this.instanceofEntitySnowBall.getId() });
