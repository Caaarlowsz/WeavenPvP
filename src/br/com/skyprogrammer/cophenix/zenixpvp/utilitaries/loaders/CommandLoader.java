...
public class CommandLoader
{
    public static void loadCommands(final JavaPlugin instanceOfJavaPlugin, final String stringOfThePackageNameToLoad) {
        ...
        int error = 0;
        try {
            for (final Class<?> classConstructor : ClassGetter.getClassesForPackage(instanceOfJavaPlugin, stringOfThePackageNameToLoad)) {
                if (Command.class.isAssignableFrom(classConstructor)) {
                    try {
                        Command instanceOfCommand = null;
                        try {
                            final Constructor<?> instanceOfClassConstructor = classConstructor.getConstructor(instanceOfJavaPlugin.getClass());
