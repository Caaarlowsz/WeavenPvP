...
public class StringAnimated {
	...
	public StringAnimated(final String stringToAnimate, final String stringColor1, final String stringColor2,
			final String stringColor3, final int sizeOfTheAnimation) {
	...
	private void createFrames(final String localString1, final String localString2, final String localString3,
			final int sizeInteger) {
		if (this.stringToAnimate != null && !this.stringToAnimate.isEmpty()) {
			for (int i = 0; i < this.stringToAnimate.length(); ++i) {
				if (this.stringToAnimate.charAt(i) != ' ') {
					this.listOfString.add(String.valueOf(localString1) + this.stringToAnimate.substring(0, i)
							+ localString2 + this.stringToAnimate.charAt(i) + localString3
							+ this.stringToAnimate.substring(i + 1));
				}
			}
			...
			for (int i = 0; i < this.stringToAnimate.length(); ++i) {
				if (this.stringToAnimate.charAt(i) != ' ') {
					this.listOfString.add(String.valueOf(localString3) + this.stringToAnimate.substring(0, i)
							+ localString2 + this.stringToAnimate.charAt(i) + localString1
							+ this.stringToAnimate.substring(i + 1));
				}
			}
			for (int i = 0; i < sizeInteger; ++i) {
