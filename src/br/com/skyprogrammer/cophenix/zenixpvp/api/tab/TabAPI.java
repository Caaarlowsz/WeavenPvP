...
public class TabAPI {
	public static void setTabTitleToPlayer(final Player playerToSet, String stringAbove, String stringBelow) {
		...
		final IChatBaseComponent firstIChatBaseComponent = ChatSerializer
				.a("{'color': 'white', 'text': '" + stringAbove + "'}");
		final IChatBaseComponent secondIChatBaseComponent = ChatSerializer
				.a("{'color': 'white', 'text': '" + stringBelow + "'}");
		localPlayerConnection.sendPacket(
				(Packet) new ProtocolInjector.PacketTabHeader(firstIChatBaseComponent, secondIChatBaseComponent));
	}
}
